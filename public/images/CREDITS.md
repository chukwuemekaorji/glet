...
Track the source and license of every image added under `public/images/`.
See MASTER.md §5 for sourcing guidance — no real, identifiable photos of
specific children from unlicensed sources (Pinterest, random search
results, etc.).

| File | Source | License | Notes |
|---|---|---|---|
| `glet-logo.jpg` | Supplied directly by the client (GLET) | Owned by GLET | Original logo file, flat white background. Kept for reference/reprocessing. |
| `glet-logo-transparent.png` | Derived from `glet-logo.jpg` | Owned by GLET | Background removed locally (white-threshold key) for use anywhere on the site — nav, footer, favicon, JSON-LD. |
...
| `programme/four-girls-reading-together.jpg` | Supplied directly by the client (GLET) | Owned by GLET | Real photo, used near the Ambassador model / peer-teaching content. |
| `get-involved/two-girls-studying-library.jpg` | Supplied directly by the client (GLET) | Owned by GLET | Real photo, used on the Get Involved page. |

Two of the ten files above (marked "unconfirmed") look AI-generated/illustrated rather than photographic — flag this to the client to confirm before any print or paid-media use, since stylized AI art can carry different usage/rights considerations than an organization's own photography.
